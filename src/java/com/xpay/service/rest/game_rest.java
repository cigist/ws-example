...
//        validate mitra account
        ...
        try {
            if (valid && !xuid.isEmpty() && !xpass.isEmpty()) {
                if (balance > price) {
                    Vector<VocuherGame> dataResponse = new Vector<>();
                    String TrnNumber = "T06" + dateCode + trnCode;
                    dataResponse = fastFay.voucherGame(xproductcode.substring(3, lengthProductCode), xnomertelepon, TrnNumber, TrnNumber,xproductcode,xuid);
